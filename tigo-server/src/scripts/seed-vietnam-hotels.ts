...
import { Room } from '../modules/hotel/entities/room.entity';
import { HotelAmenity } from '../modules/hotel/entities/hotel-amenity.entity';
import { RoomAvailability } from '../modules/hotel/entities/room-availability.entity';

// Import real Vietnamese hotel data
import { hotels as vietnamHotelsData } from './vietnam-hotels-data';
...
async function bootstrap() {
  ...
  try {
    ...
    const amenityRepository = app.get<Repository<HotelAmenity>>(getRepositoryToken(HotelAmenity));
    const availabilityRepository = app.get<Repository<RoomAvailability>>(getRepositoryToken(RoomAvailability));

    // Clear existing test hotels (in the right order to avoid FK constraints)
    ...
    // Ensure we have all necessary amenities
    const amenitiesData = [
    ...
    const createdAmenities: HotelAmenity[] = [];
    ...
    // Use real Vietnamese hotel data from crawled source
    console.log(`Creating ${vietnamHotelsData.length} authentic Vietnamese hotels...`);
    
    // Generate descriptions and ratings for the real hotels
    const getHotelDescription = (hotelName: string, city: string): { description: string, rating: number, reviews: number, basePrice: number } => {
      // Different hotel types based on names
    ...
    for (const hotelData of vietnamHotelsData) {
      ...
      // Assign random amenities (4-8 per hotel)
      ...
      for (let j = 0; j < roomTypes.length; j++) {
        ...
        const roomsCount = Math.floor(Math.random() * 4) + 3; // 3-6 rooms of each type
        ...
        for (let k = 1; k <= roomsCount; k++) {
          ...
          for (let day = 0; day < 60; day++) {
            ...
            // Random price variation (±20% from base price)
    ...
    console.log(`📊 Summary:`);
    console.log(`   - Created ${vietnamHotelsData.length} real Vietnamese hotels with authentic names and addresses`);
    console.log(`   - Cities: Ho Chi Minh City (${vietnamHotelsData.filter(h => h.city === 'Ho Chi Minh City').length}), Hanoi (${vietnamHotelsData.filter(h => h.city === 'Hanoi').length}), Da Nang (${vietnamHotelsData.filter(h => h.city === 'Da Nang').length})`);
