...
async function bootstrap() {
  ...
  // 2. Get or create HotelOwner role
  ...
  // 3. Create hotel owners (users)
  console.log('Creating hotel owners...');
  const hotelOwners: User[] = [];
  const ownerData = [
  ...
  // 4. Create 50 hotels with detailed data
  ...
  for (let i = 0; i < hotelsData.length; i++) {
    ...
    // Assign random amenities (5-10 per hotel)
    ...
    for (let j = 0; j < roomTypes.length; j++) {
      ...
      const roomsCount = Math.floor(Math.random() * 8) + 3; // 3-10 rooms of each type
      ...
      for (let k = 1; k <= roomsCount; k++) {
        ...
        if (!room) {
          ...
          for (let day = 0; day < 365; day++) {
            ...
            const priceVariation = (Math.random() - 0.5) * 0.4; // -0.2 to +0.2
